...
- **oh-my-claudecode(OMC) 5.2.0**: 2026-09-04 npm으로 설치(`npm i -g oh-my-claude-sisyphus`, `omc setup --no-plugin`). VS Code 확장에서는 `/plugin` 명령이 안 돼서 플러그인 방식 대신 npm 방식 사용. `~/.claude/`에 hooks·agents(19)·skills(35)·HUD·전역 CLAUDE.md 설치됨. tmux(psmux) 미설치라 `/team` 계열은 불가. 설치 전 settings.json 백업은 스크래치에 있음. 업데이트는 `omc update`, 제거는 `npm uninstall -g oh-my-claude-sisyphus` 후 settings.json의 hooks·statusLine 제거.
