...
**0. (사용자 지시, 2026-09-04 저녁) 노트북 실행부터 시작하지 않는다.** 다음 세션은 오늘(2026-09-04) 정리한 것들을 토대로 **주제를 정립하고, 그중 쓸 것만 추려서 확립한 뒤** 진행한다. 순서는 "이 연구를 왜 하게 됐는가 → 배경 → 어떤 데이터가 있는가 → 주제·범위 확정". Claude는 위 CLAUDE.md의 사실(경위·45편·데이터 사실·메인 3편·친구 repo 판정)을 정리해 주는 역할이며, 방향·아이디어를 먼저 제시하지 않는다(절대 금지 규칙 참조). 아래 1~5는 그 확립이 끝난 뒤의 순서.
