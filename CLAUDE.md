...
**0. (사용자 지시, 2026-09-04 저녁) 노트북 실행부터 시작하지 않는다.** 다음 세션은 "이 연구를 왜 하게 됐는가 → 배경 → 어떤 데이터가 있는가" 순서로 처음부터 천천히 계획 전체를 다시 정립하는 것으로 시작. Claude는 위 CLAUDE.md의 사실(경위·45편·데이터 사실·메인 3편·친구 repo 판정)을 정리해 주는 역할이며, 방향·아이디어를 먼저 제시하지 않는다(절대 금지 규칙 참조). 아래 1~5는 그 정립이 끝난 뒤의 순서.
